...
class Send24 extends CarrierModule
{
	...
	public function hookNewOrder($params)
	{
        ...
        if($is_available == true){
            ...
            if($select_country == $this->express){ $select_country = 'Danmark'; $where_shop_id = 'ekspres'; }
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, '
                                            ...
                                            "total": "'.$ship_total.'",
                                            "ship_mail": "'.$cookie->email.'",
                                            "bill_mail": "'.$cookie->email.'"
                                            }
                                            ');
		...
		if ($row = Db::getInstance()->getRow($sql)){
		  	$html = '<div class="panel">
						...
						<div id="messages" class="well hidden-print">
							<a class="btn btn-default" href="'.$row['link_to_pdf'].'" target="_blank"><i class="icon-file-pdf-o"></i> PDF</a>
							<a class="btn btn-default" href="'.$row['link_to_doc'].'" target="_blank"><i class="icon-print"></i> DOC</a>
							<a class="btn btn-default" href="'.$row['link_to_zpl'].'" target="_blank"><i class="icon-print"></i> ZPL</a>
							<a class="btn btn-default" href="'.$row['link_to_epl'].'" target="_blank"><i class="icon-print"></i> EPL</a>
						</div>
					</div>';
		  	return $html;
		}
	...
	public function hookHeader()
	{
		...
		if (($_SERVER['PHP_SELF'] == 'order.php' || Tools::getValue('controller') == 'order') && Tools::getValue('step') == 1 || Tools::getValue('step') == 2)
		{
			...
			if($start_time < $today && $end_time > $today){
		        ...
		        if(empty($send24_countries->errors)){
			        ...
			        for ($i = 0; $i < $n; $i++)
			        {
			            if ($send24_countries[$i]->title == $select_country)
			            {   
			                $cookie->price_express = $send24_countries[$i]->price;
							               ...
							               //self::createCarriers($res->time);
							            	 foreach ($this->_carriers as $key => $value) {
												...
												if($carriers->deleted == '1'){
													$carriers->deleted = 0;
													header('Location: /index.php?controller=order&step=3');
												}
												$carriers->update();
											}
										}else{
											// If not correct time.
											$this->delete_show_shipping();
										}
									}else{
	...
	public function delete_show_shipping(){
		foreach ($this->_carriers as $key => $value) {
			$carriers = new Carrier((int)(Configuration::get(self::PREFIX . $value)));
	...
	protected function createCarriers($eta)
	{
		...
		foreach ($this->_carriers as $key => $value) {
			...
			if ($carrier->add()) {
				...
				copy(dirname(__FILE__) . '/views/img/send24.jpg', _PS_SHIP_IMG_DIR_ . '/' . (int) $carrier->id . '.jpg'); //assign carrier logo
    ...
    public function getContent()
	{
	    ...
	    {
	        $send24_title = strval(Tools::getValue('send24_title'));
	        $send24_consumer_key = strval(Tools::getValue('send24_consumer_key'));
	        $send24_consumer_secret = strval(Tools::getValue('send24_consumer_secret'));
	        $start_work_express = strval(Tools::getValue('start_work_express'));
	        $end_work_express = strval(Tools::getValue('end_work_express'));
        	$auth = base64_encode($send24_consumer_key.':'.$send24_consumer_secret);
	        $error = false;
	        // Check title.
